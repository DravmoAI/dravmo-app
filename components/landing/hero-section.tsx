...
export function HeroSection() {
  ...
  const text = "AI Design Review Engine".split("");
  ...
  return (
    <header className="lg:h-screen flex flex-col items-center lg:justify-center text-center bg-cover bg-center bg-no-repeat relative px-4 overflow-hidden">
      <Image
      ...
      <motion.h1
        ...
        initial="hidden"
        animate="visible"
        className="text-[14px] lg:text-[20px] leading-[16px] lg:leading-[20px] text-center tracking-[0.3em] lg:tracking-[0.5em] uppercase text-[#F7EDE2] font-quantico -translate-y-16 lg:-translate-y-32 relative z-10 px-4 mt-3 flex overflow-hidden"
      >
        {text.map((char, index) => (
          <motion.span className="font-quantico" variants={childVariants} key={`${char}-${index}`}>{char === " " ? "\u00A0" : char}</motion.span>
        ))}
      </motion.h1>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="hidden lg:flex flex-col lg:flex-row items-center justify-center gap-y-4 lg:gap-y-0 lg:gap-x-[20px] -translate-y-12 lg:-translate-y-24 relative z-10 px-4">
        <Button
          variant="outline"
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex lg:hidden flex-col items-center justify-center gap-y-6 -translate-y-8 relative z-10 px-4">
        <Button
          variant="outline"
          onClick={() => router.push("/login")}
