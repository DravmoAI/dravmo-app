...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch("http://localhost:8000/general_review", {
      method: "POST",
      headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json({ error: "Failed to generate review with AI backend" }, { status: 500 })
    }
