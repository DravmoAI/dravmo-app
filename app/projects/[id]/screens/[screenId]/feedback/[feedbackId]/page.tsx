...
export default function FeedbackDetailPage() {
  ...
  const getSeverityFromJudgement = (judgement: string): "high" | "medium" | "low" => {
    ...
    if (
      lowerJudgement.includes("critical") ||
      lowerJudgement.includes("major") ||
      lowerJudgement.includes("serious")
    ) {
      return "high";
    } else if (
            ...
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center">
                  {expertReview.name && feedbackResult?.feedbackQuery.designMaster?.avatarUrl ? (
                    <Image
                      width={40}
                      height={40}
                      alt={expertReview.name}
                      className="rounded-full"
                      src={feedbackResult?.feedbackQuery.designMaster?.avatarUrl}
                    />
                  ) : (
        ...
        >
          {Object.entries(aiFeedback.general_review).map(([topicName, topicData]) => (
            <AccordionItem key={topicName} value={topicName}>
              <AccordionTrigger className="text-lg font-medium font-krona-one">
                {topicName}
              </AccordionTrigger>
              <AccordionContent>
                    ...
                    <div key={subtopicName} className="space-y-3">
                      ...
                      {Object.entries(subtopicData).map(([pointName, judgements]) => (
                        <div key={pointName} className="space-y-2">
                          <h5 className="font-medium text-sm font-quantico text-muted-foreground">
                            {pointName}
                          </h5>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <h2 className="text-3xl font-bold font-krona-one">
                Feedback {feedbackResult.version}
              </h2>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-6 space-y-6">
              <div className="flex gap-2 border-b">
                ...
                >
                  {aiFeedback?.expert_reviews && aiFeedback.expert_reviews.length > 0
                    ? "Master Review"
                    : "AI Feedback"}
                </button>
                <button
                  ...
                  <Card>
                    <CardContent className="p-6">
                      ...
                      <div className="grid grid-cols-1 gap-4 text-sm">
                        <div>
                          <span className="font-medium text-muted-foreground font-quantico">
                            Industry:
                          </span>
